...
### 4.1. page.tsx 말고 `route.ts 생성`에 대한 이해

- page.tsx 는 화면의 UI 출력
- Next.js 에서는 route 만 처리하는 파일명이 별도로 존재함.
...
- Next.js 는 서버라서 웹브라우저에 접근이 불가하다.
- 즉, 웹브라우저에 보관해주어야 할 Supabase 쿠키를 생성못함.
...
- /src/lib/supabase/server.ts : Next.js 서버 컴포넌트에서 활용(서버용), 비동기(await)
- /src/lib/supabase/middleware.ts : Next.js 와연동하는 경우 활용, 동기

## 6. supabase 의 middleware.ts
...
- 파일의 위치가 고정이 되어있음. (`/middleware.ts` 고정)
...
- `/src/hooks/mutations/useRequestPasswordResetEmail.ts`
