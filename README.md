...
- onSuccess 이벤트 핸들러에서 캐시 데이터를 수정하고 즉시 랜더링 하도록 구성
...
- `/src/hooks/todos/mutations/useCreateMutations.ts 파일` 생성
...
export default function useCreateMutation() {
  ...
  return useMutation({
    ...
    onSuccess: () => {
      ...
      // 2. 아래 코드는 데이터를 무효화 시켜서 리패칭을 실행한다.
...
export const useTodoDataById = (id: number) => {
  return useQuery({
    ...
    staleTime: 5000, // 5초동안 fresh 유효기간
    gcTime: 10000, // 10 초동안 inactive 유효기간
  });
};
```
