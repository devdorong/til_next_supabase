...
- combine(state 객체, 콜백함수)
- `state 객체의 타입을 자동 추론`해 준다.
- 전체 Store 의 타입을 추론하는 건 아니다.
- 정확히 개발자가 명시해 주기 위해서 action 은 아래처럼 작성한다.
...
useCountStore.subscribe(
  ...
  (count, prevCount) => {
    ...
    console.log(store);
    // 아래 코드는 store 의 count 를 계속 업데이트 하기 때문에 무한 루프가 돕니다.
    // 현재 스토어의 값을 업데이트 하기
...
```ts
const useCountStore = create(persist(subscribeWithSelector 함수등, {옵션객체}));
```
...
- 문제점 확인해 보기
  <img width="1630" height="251" alt="Image" src="https://github.com/user-attachments/assets/81a734e4-042f-46eb-9037-d5bf36bd6174" />
